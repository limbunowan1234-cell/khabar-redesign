#!/usr/bin/env node
// Post-cutover check (Week 27): follows write to D1 only now -- Appwrite's
// follows collection is frozen at whatever it held at cutover time. Same
// flip as diff-likes.mjs/diff-bookmarks.mjs:
//   - "Only in Appwrite" should now stay flat forever (real drift if it
//     grows -- would mean something's still writing there).
//   - "Only in D1" is now the expected, growing bucket for everything
//     that happens from here on -- not a bug signal.
//
...
async function main() {
  ...
  console.log(`Only in Appwrite (frozen since Week 27 cutover -- should stay flat; growing = real drift, investigate): ${onlyInAppwrite.length}`);
  console.log(`Only in D1 (expected and growing since Week 27 -- every follow/unfollow since cutover only lands here): ${onlyInD1.length}`);
